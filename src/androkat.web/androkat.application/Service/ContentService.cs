...
public class ContentService : IContentService
{
	...
	{
		var contentDetailsModel = GetMainCache().FirstOrDefault(w => w.Tipus == tipus); // && w.Nid == nid
	...
	private IEnumerable<ContentDetailsModel> Get(int[] tipusok)
	{
		...
		return tipusok.Length > 0 ? result.Where(w => tipusok.Contains(w.Tipus)) : result;
	}
	...
	private List<ContentDetailsModel> GetMainCache()
	{
		var res = Get(Array.Empty<int>());

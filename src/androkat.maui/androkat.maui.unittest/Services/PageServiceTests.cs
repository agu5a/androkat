    ...
    public async Task Test_GetContentsAsync(string pageTypeId, string methodName)
    {
        ...
        switch (methodName)
        {
            case "GetContentsByTypeId":
                if (pageTypeId == "1")
                {
                    repositoryMock.Verify(x => x.GetContentsByTypeId(((int)Activities.ajanlatweb).ToString(), It.IsAny<bool>(), It.IsAny<List<string>?>()), Times.Once);
                }
                else if (pageTypeId == "2")
                {
                    repositoryMock.Verify(x => x.GetContentsByTypeId(((int)Activities.maiszent).ToString(), It.IsAny<bool>(), It.IsAny<List<string>?>()), Times.Once);
                }
                else if (pageTypeId == "11")
                {
                    repositoryMock.Verify(x => x.GetContentsByTypeId(((int)Activities.book).ToString(), It.IsAny<bool>(), It.IsAny<List<string>?>()), Times.Once);
                }
                else if (pageTypeId == "34")
                {
                    repositoryMock.Verify(x => x.GetContentsByTypeId(((int)Activities.gyonas).ToString(), It.IsAny<bool>(), It.IsAny<List<string>?>()), Times.Once);
                }
                break;
